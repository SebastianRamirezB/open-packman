...
function drawGhost( ctx, g, color, frame, frightTimer ) {
  let bodyColor = color;
  let eyeColor = '#0000bb';
  let pupilColor = '#0000bb';
  if ( g.frightened ) {
    bodyColor = '#2121ff';
    eyeColor = '#fff';
    pupilColor = '#fff';
    // Parpadeo a blanco en los ultimos 120 frames del timer.
    if ( frightTimer < 120 && frame % 16 < 8 ) {
      bodyColor = '#fff';
      pupilColor = '#ff0000';
    }
  }
...
const GHOST_COLORS = {
  hunter: '#ff0000',       // rojo  (Blinky)
  ambush: '#ffb8ff',      // rosa  (Pinky)
  flank: '#00ffff',       // cian  (Inky)
  intermittent: '#ffb852', // naranja (Clyde)
};
...
function draw( ctx, game, frame ) {
  ...
  game.ghosts.forEach( ( g ) => drawGhost( ctx, g, GHOST_COLORS[ g.kind ] || '#ff0000', frame, game.frightTimer ) );
  drawHUD( ctx, game, W );
}
